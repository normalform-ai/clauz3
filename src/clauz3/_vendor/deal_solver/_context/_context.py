...
from ._layer import ExceptionInfo, FactInfo, Layer, ReturnInfo
...
class Context(typing.NamedTuple):
    ...
    facts: Layer[FactInfo]              # trusted function invocations
    quantifiers: Layer[Quantifier]      # for-loop quantifier frames
    ...
    get_guarantees: typing.Callable[[typing.Any], typing.Iterator]

    @classmethod
    def make_empty(cls, *, get_contracts, get_guarantees=None, **kwargs) -> Context:
        if get_guarantees is None:
            get_guarantees = _empty_guarantees
        obj = cls(
            z3_ctx=None,
    ...
    def make_child(self) -> Context:
        return self.evolve(
            ...
            facts=self.facts.make_child(),
            quantifiers=self.quantifiers.make_child(),
        )


def _empty_guarantees(_func) -> typing.Iterator:
    return iter(())
