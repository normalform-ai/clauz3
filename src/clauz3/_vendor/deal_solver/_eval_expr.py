...
def eval_name(node: astroid.Name, ctx: Context) -> ProxySort:
    ...
    # resolve Row subclass names via sys.modules (for contract arguments)
    row_class = _resolve_row_class(node)
    if row_class is not None:
        from ._proxies._row import RowClassSort

        return RowClassSort(schema=row_class)

    raise UnsupportedError("cannot resolve name", node.name)


def _resolve_row_class(node: astroid.Name) -> type | None:
    """Try to resolve a Name node to a clauz3.Row subclass via sys.modules.

    Returns the class if found and it is a Row subclass, otherwise None.
    Trusted modules are pre-loaded before proof evaluation, so any Row
    subclass with this name will be present in sys.modules at call time.
    """
    import sys

    from clauz3.row import Row

    # Search all loaded modules for a class with this name that is a Row subclass.
    # Trusted modules are loaded before proof evaluation begins, so this finds
    # UserRow (and similar) even when the guarantee source was parsed from text
    # (where the astroid root module has an empty name).
    for mod in list(sys.modules.values()):
        if mod is None:
            continue
        cls = getattr(mod, node.name, None)
        if isinstance(cls, type) and issubclass(cls, Row) and cls is not Row:
            return cls
...
def eval_call(node: astroid.Call, ctx: Context) -> ProxySort:
    ...
    target_name = node.func.as_string()
    func = FUNCTIONS.get(target_name)
    if func is not None:
        return FuncSort(func).m_call(*call_args, ctx=ctx, **call_kwargs)
