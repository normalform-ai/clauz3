    ...
    def __iter__(self) -> typing.Iterator[tuple[str, object]]:
        for decl in self._model.decls():
            ...
            if isinstance(z3_val, z3.FuncInterp):  # pragma: no cover
                continue
            try:
                py_val: object = eval(repr(z3_val), GLOBALS)
            except (NameError, SyntaxError):
                # Z3 datatype values (e.g., `mk_userrow("Bob", ...)`) use
                # constructor names that aren't in GLOBALS. Fall back to the
                # Z3 string representation so the counterexample stays
                # legible instead of raising.
                py_val = str(z3_val)
            yield name, py_val
